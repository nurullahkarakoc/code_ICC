...
M=100e3;
TSDN_delay=500;  % TSDN = TSDN_delay*2 + TSDN_iter
TSDN_iter=1e3;
TOP_delay=50;
TOP_iter=100;
TGW_delay=5;
...
for tt=1:tot_params % parfor for parallel computation
    disp(tt)
    meanAlloc=zeros(M/(2*TGW_delay),Num_O);
    meanqG1=zeros(M/(2*TGW_delay),Num_O);
    meanqG2=zeros(M/(2*TGW_delay),Num_O);
    meanUti=zeros(M/(2*TGW_delay),1);
    for count=1:5
        ...
        utility=zeros(M/(2*TGW_delay),1);
        allocations=zeros(M/(2*TGW_delay),Num_O);
        q1=zeros(Num_E,Num_G);
        q2=zeros(Num_E,Num_G);
        qG1_Opt=zeros(M/(2*TGW_delay),2);
        qG2_Opt=zeros(M/(2*TGW_delay),2);
        for mm=1:M
            ...
            w_s=demands;
            %Upper level
            if(mod(mm,TSDN_iter+2*TSDN_delay)<TSDN_iter)
                %Update K
                %new lambda_K in
                if(mod(mm,TSDN_iter+2*TSDN_delay)==0)
                ...
                if(mod(mm,TOP_iter+2*TOP_delay)==0)
            ...
            if(mod((mm-TOP_delay),TOP_iter+2*TOP_delay)<TOP_iter)   %check when mm negative
                if(mod(mm-TOP_delay,TOP_iter+2*TOP_delay)==0)
                    %new lambda_G in
                    for oo=1:Num_O
            ...
            if(mod(mm-TGW_delay,2*TGW_delay)==0)
                if(mm==TGW_delay)
                    avg_w_s_new=avg_w_s_new/TGW_delay;
                    demands_received(:,:,1)=max(demands_received(:,:,1)-gamma(:,:,1)+sum(demandsOp1(:,:,mm-TGW_delay+1:mm),3)/TGW_delay,0);
                    demands_received(:,:,2)=max(demands_received(:,:,2)-gamma(:,:,2)+sum(demandsOp2(:,:,mm-TGW_delay+1:mm),3)/TGW_delay,0);
                else
                    avg_w_s_new=avg_w_s_new/(2*TGW_delay);
                    demands_received(:,:,1)=max(demands_received(:,:,1)-gamma(:,:,1)+sum(demandsOp1(:,:,mm-(2*TGW_delay)+1:mm),3)/(2*TGW_delay),0);
                    demands_received(:,:,2)=max(demands_received(:,:,2)-gamma(:,:,2)+sum(demandsOp2(:,:,mm-(2*TGW_delay)+1:mm),3)/(2*TGW_delay),0);
                end
             for oo=1:Num_O
                    for ss=1:Num_G
            ...
            %%disp(mm)
            if(mod(mm,(2*TGW_delay))==0)
                qG1_Opt(floor(mm/(2*TGW_delay)),:)=sum(q(:,:,1));
                qG2_Opt(floor(mm/(2*TGW_delay)),:)=sum(q(:,:,2));
                allocations(floor(mm/(2*TGW_delay)),:)=reshape(sum(sum(gamma,1),2),[1,Num_O]);
                if(linear_utility)
                    utility(floor(mm/(2*TGW_delay)))=sum(sum(sum(w_s.*gamma)));
                else
                    utility(floor(mm/(2*TGW_delay)))=sum(sum(sum(w_s.*log(1+gamma))));
                end
            end
        end
