...
allocations=zeros(interval,Num_O);
                    ...
                    for l=1:Num_G % For each eNB's
                        ...
                        for m=1:Num_iter3 % Updating G's
                            for j=1:Num_iter % Updating lamdas
                                ...
                                lamdas(j)=lamda;
                                for i=1:Num_E  % For each eNB
%                                     if (w_s(i,l,p)<lamda)
%                                         gammas(i,l,p)=0;
%                                     else
%                                         gammas(i,l,p)=demands(i,l,p);
%                                     end
                    ...
                    summm(p)=sum(summ(:,p));
                    allocations(s,p)=summm(p);
                end
                Ko(p)=max(Ko(p)+alpha5*(nu-queue(p)-mu),0);
                queue(p)=max(queue(p)+(Ko(p)-Kind(p)),0);
            end
        end
