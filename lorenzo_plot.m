...
% end
load('lorenzo_28_9_waiting_scheme_finer_count_50.mat')
%load('lorenzo_plot_ready_27_9.mat')
%%
cardV=length(V_vec);
...
aa=2;
filelist=[{'allocation_1_V_1_d_0'},{'allocation_2_V_1_d_0'},{'allocation_1_V_1e3_d_0'},{'allocation_2_V_1e3_d_0'};
          {'allocation_1_V_1_d_7'},{'allocation_2_V_1_d_7'},{'allocation_1_V_1e3_d_7'},{'allocation_2_V_1e3_d_7'};
          {'allocation_1_V_1_d_13'},{'allocation_2_V_1_d_13'},{'allocation_1_V_1e3_d_13'},{'allocation_2_V_1e3_d_13'}];
%dist_idx_vec=[1,7,14];
dist_idx_vec=13;
% for aa=1:cardA;
%    figure(aa)
%figure(1)
    ...
    idx_to_test=sub2ind([cardV,cardD,cardA],[1,4],dist_idx*ones(1,2),aa*ones(1,2));
    T=1e1;
    ...
    saveDat([[10:10:M]',movmean(allocations_database{idx_to_test(jj)}(:,1),T)],char(filelist(dd+2,2*(jj-1)+1)))
    %plot([10:10:M],movmean(allocations_database{idx_to_test(jj)}(:,2),T),'LineWidth',2.0);
    saveDat([[10:10:M]',movmean(allocations_database{idx_to_test(jj)}(:,2),T)],char(filelist(dd+2,2*jj)))
    end
    end
...
for vv=1:4
    for ii=1:21
        avg_over_d1_Opt(ii,vv)=mean(mean(qG1_Opt_database{vv+(ii-1)*cardV+(aa-1)*cardV*length(iter_vec)}));
        avg_over_d2_Opt(ii,vv)=mean(mean(qG2_Opt_database{vv+(ii-1)*cardV+(aa-1)*cardV*length(iter_vec)}));
%         if(vv==1)
...
% % figure(2)
% % for jj=1:length(idx_to_test)
% %     plot([1:M],movmean(sum(qG1_Opt_database{idx_to_test(jj)},2),T))
% %     hold on
% %     plot([1:M],movmean(sum(qG2_Opt_database{idx_to_test(jj)},2),T))
