...
alpha_vec=.5;
% iter_vec=[10,20,30];
%iter_vec=[0:M/80:M/4]; % I am using as input peak distance for now.
iter_vec=M/10;
beta=0.2;
% V_vec=[.1,1,10];
%V_vec=[1e-2,1,1e2];
%V_vec=[1,1e2,100,1e3,1e4];
V_vec=100/16*1e3;
...
%     meanUti=zeros(M,1);
 %   for count=1:1
  %      disp(count)
        ...
        alpha=200/64*input_params{tt}(3); % Step sizes of algorithms
        Num_iter2=Num_iter;
        Num_iter3=Num_iter;
        Num_iter4=Num_iter;
        alpha_2=100/16*alpha;
        alpha_3=200/4*alpha;
        alpha_4=10*alpha;
        utility=zeros(M,1);
        demands=zeros(Num_E,Num_G,Num_O);
        q=zeros(Num_E,Num_G,Num_O);
        avg_demands_new=zeros(Num_E,Num_G,Num_O);
        avg_w_s_new=zeros(Num_E,Num_G,Num_O);
        Kbar=.5*Num_E*Num_G*Num_O;
        Ko=.5*(Num_E*Num_G)*ones(1,Num_O);
        Ko_proj=Ko;
        Kbar_o=Ko;
        ...
        for mm=1:M
          ...
          demands(:,:,2)=demandsOp2(:,:,mm)+q(:,:,2);
            ...
            if(mod(floor(mm/1e3),2)==0)
                ...
                if(mod(mm,2e2)==0)  %changed from 1e2 to 2e2
...
%     utility_database{tt}=meanUti;
    %     disp(tt)
%end
%save('lorenzo_dyn_9_27_opt_finer_trial.mat')
% plot(utilities)
% figure;
% plot(lambdas)
% figure;
